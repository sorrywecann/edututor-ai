...
function ensureHfCacheExtracted() {
  return new Promise((resolve) => {
    if (!app.isPackaged) return resolve();
    const src = join(process.resourcesPath, 'backend', 'hf-cache');
    ...
    // dir wiped by the user, a cleaner, or AV quarantine. Without this check,
    // backend boots, HF_HOME points at an empty dir, MiniLM does 30+ HEAD
    // requests, and if offline crashes ~60s into startup with no clear log.
    ...
    if (!existsSync(src)) {
      console.warn('[hf-cache] bundled source missing:', src, '— backend will download from HF on first use');
      return resolve();
    }
    splashStatus('Pripravujem AI model…');
    try { rmSync(dest, { recursive: true, force: true }); } catch { /* ignore */ }
    try {
      // v0.6.4: dereference:false preserves the HF hub layout (snapshots/*/*
      // are symlinks/hardlinks/reparse-points into blobs/<sha>). With
      // dereference:true (the old default), huggingface_hub's integrity
      // check sees regular files where it expected links and HEAD-revalidates
      // every snapshot against the live hub — which fails offline on first
      // STT call. Same risk for backend Python's downstream copy if it ever
      // re-stages the cache.
      cpSync(src, dest, { recursive: true, dereference: false, verbatimSymlinks: true });
      writeFileSync(marker, version);
      console.log('[hf-cache] extracted →', dest);
    } catch (e) {
