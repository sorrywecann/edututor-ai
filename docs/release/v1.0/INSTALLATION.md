...
### Quick start — auto-download (recommended for fresh contributors)
```powershell
.\start.ps1 -Avatar
```
First run downloads ~1.77 GB (UE5 + Wilbur) to %LOCALAPPDATA%\edututor\avatar\.
Subsequent runs are instant (cache hit).

### Team workflow — sibling UE5 clone
```powershell
git clone -b Edutor_UnrealEngine <ue5-repo> ..\edotutor-ue5-latest
.\start.ps1 -Avatar -UseSiblingClone
```
Skips download, uses local UE5 source for instant Blueprint iteration.
