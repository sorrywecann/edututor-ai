...
- 0.3 Verify main is pushed: cd edututor-ai-sandbox-test && git status && git push origin main. Expect clean working tree.
...
- 3.6 ADVERSARIAL FIX (HIGH - apps/desktop/main.mjs UE5_RELEASE_REPO): Update apps/desktop/main.mjs and orchestrator.mjs - replace 'princeofwellness/edututor-ai-sandbox-releases' with 'sorrywecann/edututor-ai' in UE5_RELEASE_REPO constant. Verify: grep -rn 'princeofwellness|edututor-ai-sandbox-releases' apps/desktop/ -> zero matches.
...
- 5.2 Rewrite README.md per master plan outline: remove grant attribution (line 8), SORRYWECAN footer (line 438), replace ALL princeofwellness/edututor-ai-sandbox URLs with sorrywecann/edututor-ai. Add badges, quick start, features matrix, link to ARCHITECTURE.md.
...
- 5.9 Create CHANGELOG.md fresh: entry 'v1.0.0 - Initial public release - Slovak AI language tutor with multi-provider LLM, voice, RAG, cross-session memory, optional UE5 MetaHuman avatar.' Add migration note: 'Users of v0.8.x from princeofwellness/edututor-ai-sandbox-releases must download v1.0.0 manually.'
...
- 9.5 Archive princeofwellness/edututor-ai-sandbox-releases (if BLOCKER 5 = hard cut): add README banner pointing to sorrywecann/edututor-ai, optionally mark repo as archived (Settings -> Danger Zone -> Archive). Old v0.8.x releases remain downloadable.
...
**Verification (must pass before next phase):**
- `gh api /repos/princeofwellness/edututor-ai-sandbox/branches -> 9 branches remain (matches keep-list)`
...
**Blocker questions:**
- Should princeofwellness/edututor-ai-sandbox-releases be archived now or kept active for legacy v0.8.x downloads?
...
| Phase 9 - Private Cleanup | `gh api /repos/princeofwellness/edututor-ai-sandbox/branches | ConvertFrom-Json | Measure-Object`<br>`Test-Path <repo-root>/edututor-ai-sandbox-test/docs/PUBLIC_RELEASE_CHECKLIST.md`<br>`Get-Content <repo-root>/.claude/projects/C--Users-<owner>/memory/MEMORY.md | Select-String 'sorrywecann/edututor-ai'` | Private repo has 9 branches (14 stale deleted, 9 kept); PUBLIC_RELEASE_CHECKLIST.md exists in private repo; memory file references new public repo URL |
...
Based on _
**Concern:** main.mjs UE5_RELEASE_REPO hardcoded reference is mentioned in the brief but NOT verified in the plan. If hardcoded to princeofwellness/edututor-ai-sandbox-releases, v1.0.0 users will fetch UE5 from the old repo,
...
**Mitigation:** In apps/desktop/main.mjs (or wherever ensureUE5Downloaded is defined), confirm the repo URL is parameterized or hardcoded to sorrywecann/edututor-ai before Phase 7 commit. Test on a clean VM: ensureUE5Downloaded() should fetch from sorrywecann/edututor-ai, not princeofwellness/edututor-ai-sandbox-releases.
...
- ❗ USER DECISION 5: Backwards-compatibility for v0.8.x users on princeofwellness/edututor-ai-sandbox-releases - hard cut at v1.0.0 (document in CHANGELOG) OR dual-publish v1.0.0-v1.0.2 to both old and new release streams? Recommend hard cut + clear CHANGELOG note.
...
- Private repo (princeofwellness/edututor-ai-sandbox) has 14 stale branches deleted and retains 9 active branches (main + 5 fix/v0.x + feat/floating-glass-bars + docs/w7-security-recon + Edutor_UnrealEngine + Edutor_UnrealEngine-pow-face).
