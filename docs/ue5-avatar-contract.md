...
**For:** Roland Vraník (UE5), Martin Ivanič (3D), Adrián Putz (Rig)
...
On AvatarCommand received:
  ...
  IF audioPositionMs >= 0  (v4.0 client):
    ...
    frameStepMs = ActiveTimeline[1].start_ms - ActiveTimeline[0].start_ms  // dynamic; server default is 40ms but tunable via EDU_VISEME_FRAME_STEP_MS (Roland 2026-05-12 feedback)
